...
- 修复导入歌曲完成全部分析后，自动排序请求可能因分析阶段重叠而被静默丢弃的问题。
- 自动排序改为必达队列任务；MuQ 未启用或自动分析失败时，自动回退到 BPM/结构图搜索排序。
- 抽取统一的队列快照排序函数，MuQ 与降级路径共享相同的播放前缀保护、图搜索和相邻边评分。
- 排序完成后保留选中歌曲并滚动到可见位置；日志明确说明顺序是否实际改变。
- 新增 4 项自动重排回归测试；完整测试 98 项通过。

## 1.2.18

- 简化/尾端转场新增低、中、高三个频段的重叠相关性测量，并按期望功率中的交叉项自适应修正 fader 曲线。
- 正相关素材的中点能量鼓包最多自动削减约 3 dB；首尾端点保持原值，负相关不做危险增益。
- 尾端安全路径不再套用常规 cue 前 4 拍可行性门槛，样本 0 或第一拍的 CUE-DETR IN cue 可以正常参与。
- 尾端 IN cue 增加“可独立承接”评分：综合 neural cue 置信度、后续音频空间、局部 RMS、onset 和 vocal 风险。
- 10 首及以下播放列表改用精确子集动态规划；大队列 beam search 使用两步前瞻和末端节点多样性保留。
- 修复能量轨迹符号逻辑：受控升降反转得到轻度奖励，连续大幅同向变化受到惩罚，并保留加速度限制。
- 新增 4 项研究驱动算法测试；完整回归 94 项通过，三项独立音频连续性检查通过。

## 1.2.17

- 新增端点安全 `Tail Crossfade`：最后一个 OUT cue 已错过、cue 后不足 1/4 拍或 cue 落在文件末端时，直接使用当前歌曲真实尾部等功率淡出到下一首。
- 尾端计划的开始位置不会落入已播放音频；预加载完成过晚时自动缩短左侧窗口，而不是安装过期 transition。
- 下一首 IN cue 若错误地落在文件末端，仅在端点安全路径中回退到首个有效 downbeat/样本 0，避免下一首刚进入就结束。
- 端点安全优先于 `Always DJ`，禁止无 post-cue 音频时进入鼓循环/HPSS 复杂渲染器。
- 播放列表图/beam 搜索改用 bit-mask 状态、方向邻接预排序、一层未来连通性前瞻和 Pareto 状态去重。
- 搜索完成后增加确定性 swap/relocate 局部改良，固定首曲并修复路径内部的弱边。
- 新增尾端 CUE、实时 callback 无零间隙和图路径局部改良 3 项专项测试；完整回归 90 项通过。

## 1.2.16

- 默认 CUE-DETR 窗口扩展为 cue 前 4 拍、cue 后 2 拍；新增的时间只用于量化鼓循环铺垫，不延长双旋律/双人声/双低频重叠。
- 从下一首 CUE 当拍开始提取 2 拍 percussion loop，并在 cue 前 4 拍内重复两次；循环起止点严格按 BeatGrid 整拍量化。
- 鼓循环先在约 0.75 拍内淡入并保持约 24% 电平，正式鼓组在 cue 前约 1.5 拍开始接管，cue 当拍下一首鼓组保持主导。
- 循环边界和 loop→live B 接点加入约 4 ms 周期桥接，避免重复播放产生点击或瞬时断裂。
- 早段桥接只使用 percussion stem；下一首 harmonic 与 bass 仍在 CUE 附近进入，bass 继续满足 `bass_A + bass_B = 1`。
- Simple Crossfade / Gapless 继续使用同一 CUE 与 6 拍窗口，但不会伪造鼓循环；高级自然转场失败时仍保持连续渐变和 MIX END unity。
- GUI、诊断指标与文档更新为“4 拍鼓循环桥 + 2 拍释放”；新增 4 项鼓循环专项测试和独立检查脚本。
- 完整测试增至 87 项；鼓循环、自然转场和 MIX END 连续性检查全部通过。

## 1.2.15

- 默认 CUE-DETR 可听窗口由 2 拍扩展为 4 拍：cue 前 2 拍预入/融合，cue 后 2 拍释放/稳定。
- 主旋律和声采用约 2–2.5 拍 equal-power 交叉，鼓组采用约 2 拍交叉，不再在不足一拍内完成全部切换。
- cue 当拍下一首仍保持主导，但上一首鼓组约 0.75 拍、主体约 1.1 拍后才归零，避免“完全没有过渡”的听感。
- bass 继续保持单一所有权，但交换宽度放宽到约 0.65–0.9 拍，并略微提前偏向下一首。
- Simple Crossfade / Gapless 回退曲线同步改为 4 拍平滑时序，并保持 MIX END unity 与 24 ms 接缝。
- GUI 与文档更新为“cue 前后各约 2 拍”；新增平衡交接、可听重叠和无包络空洞测试。
- 完整测试增至 83 项；自然转场和 MIX END 独立检查继续通过。
