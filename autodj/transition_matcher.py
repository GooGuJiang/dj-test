...
def _songformer_role_compatibility(
    current_role: str,
    next_role: str,
...
def _score_candidate(
    ...
    role_compatibility = _songformer_role_compatibility(
        current_role, next_role, current_function, next_function
    )
    boundary_a = (
        float(current.structure.songformer_boundary_score[current_index])
        if current.structure.songformer_boundary_score.size > current_index
        else 0.50
    )
    boundary_b = (
        float(next_track.structure.songformer_boundary_score[next_index])
        if next_track.structure.songformer_boundary_score.size > next_index
        else 0.50
    )
    songformer_boundary = float(np.sqrt(max(boundary_a, 0.0) * max(boundary_b, 0.0)))
    ...
    metrics = {
        ...
        "songformer_boundary": songformer_boundary,
        "songformer_role_compatibility": role_compatibility,
